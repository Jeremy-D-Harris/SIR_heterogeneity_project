...
% Version: Gaussian (Uncorrelated/Correlated) with Truncations
...
bet=2*gam; %change bet to match exponential growth rates

%Change manually to ensure that the calculated means equal 1
diag_mean = 0.4; %0.925
delta_S = diag_mean; % starting mean transmissibility of susceptible population
delta_I =diag_mean; % starting mean transmissibility of infectious population
epsilon_S =diag_mean;% starting mean susceptibility of susceptible population
epsilon_I =diag_mean;
...
% corr_coeff = -0.8;
Sigmas = [1 0.8; 0.8 1]; %change off-diagonals for correlation
...
%% Plotting
f7=figure(7); set(f7, 'Position', [900   50   400   620]);
...
% legend(q,{'Correlated','Classic'},'Location','SouthEast');
...
%save('dataFigure2HighVariance.mat','params','data','params_iv', 'params_v')