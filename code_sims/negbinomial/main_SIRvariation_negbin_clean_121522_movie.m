...
subplot(3,1,2);
r(1)=plot(params.t_span, mu_epsilon_S_traj,'-','Color',my_rgb_colors(1,:),'LineWidth',2); hold on;
r(2)=plot(params.t_span, mu_delta_I_traj,'-','Color',my_rgb_colors(2,:),'LineWidth',2); hold on;
...
%     open(v1);
    ...
    %     this_ind = 2;
    for this_ind = 1:10:length(t_span)
        
        fig = figure('Visible','on');
        
        subplot(2,2,1);
        plot(t_span,mu_epsilon_S_traj,'Color',my_rgb_colors(1,:),'LineWidth',2); hold on;
        plot(t_span(this_ind)*ones(size(t_span)),linspace(0,1.2,length(t_span)),'k--','LineWidth',1); hold on;
...
%         f.FontName = 'Times';
        ...
        subplot(2,2,2);
        plot(t_span,mu_delta_I_traj,'Color',my_rgb_colors(2,:),'LineWidth',2); hold on;
        plot(t_span(this_ind)*ones(size(t_span)),linspace(0,2,length(t_span)),'k--','LineWidth',1); hold on;
        
        xlabel('Time (days)');
        ylabel({'Mean Transmissibility, $\bar{\delta}$'},'interpreter','latex');
        axis([0 t_end 0 2]);
        set(gca,'LineWidth',1,'FontSize',14, 'FontWeight','normal','FontName','Times');
%         f=gca;
%         f.LineWidth = 1;
...
%         f.FontName = 'Times';
        ...
        title('Joint Distribution in $I$','interpreter','latex');
