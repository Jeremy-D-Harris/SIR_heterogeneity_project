...
eps_perturb = 1.25e-6; %change as needed
...
% init_mean_eps_S = sum(eps.*init_marg_eps_S);
% init_mean_delta_S = sum(del.*init_marg_delta_S);
% init_mean_eps_I = sum(eps.*init_marg_eps_I);
% init_mean_delta_I = sum(del.*init_marg_delta_I);
%
% %calculated values
% init_covariance = sum(sum(((eps' - init_mean_eps_S*ones(size(eps')))*(del- init_mean_delta_S*ones(size(del)))).*init_joint_S));
%
% init_variance_eps = sum((eps - init_mean_eps_S*ones(size(eps))).^2.*init_marg_eps_S);
% init_sd_eps = sqrt(init_variance_eps);
%
% init_variance_delta = sum((del- init_mean_delta_S*ones(size(del))).^2.*init_marg_delta_S);
% init_sd_delta = sqrt(init_variance_delta);
%
% corr_coef = init_covariance/(init_sd_eps*init_sd_delta);


% init_S_eps_delta_values = (dx)^2*init_joint_S*(S_init); %joint probability matrix S_{i,j}
% params.init_S_eps_delta_values = init_S_eps_delta_values;
%
% init_I_eps_delta_values = (dx)^2*init_joint_I*(I_init);
% params.init_I_eps_delta_values = init_I_eps_delta_values;
%
% init_R_eps_delta_values = (dx)^2*init_joint_S*(R_init);
...
% marginal_eps_S_traj = dx*reshape(sum(S_traj_eps_delta_array,3),length(t_span),m)./S_traj;
% marginal_delta_S_traj = dx*reshape(sum(S_traj_eps_delta_array,2),length(t_span),n)./S_traj;
% marginal_eps_I_traj = dx*reshape(sum(I_traj_eps_delta_array,3),length(t_span),m)./I_traj;
% marginal_delta_I_traj = dx*reshape(sum(I_traj_eps_delta_array,2),length(t_span),n)./I_traj;

% epsilon_S_dist_traj_fm = dx*sum(eps.*marginal_eps_S_traj,2);
% mean_epsilon_S_traj_fm = dx*sum(epsilon_S_dist_traj_fm,2);
for kk=1:length(t_span)
    ...
    % this_joint_S(:,:) = reshape(S_traj_eps_delta_array(kk,:,:),m,n)/S_traj(kk)/dx/dx;
    ...
    % mean_epsilon_S_traj(kk) = dx*dx*sum(sum(Eps.*this_joint_S));
    % mean_epsilon_S_traj(kk) = dx*sum(sum(Eps.*this_S_eps_delta_array));
    
    
    this_I_eps_delta_array(:,:) = I_traj_eps_delta_array(kk,:,:);
        
    this_joint_I = this_I_eps_delta_array/I_traj(kk)/dx/dx;
    % this_joint_I(:,:) = reshape(I_traj_eps_delta_array(kk,:,:),m,n)/I_traj(kk)/dx/dx;
    ...
    % mean_delta_I_traj(kk) = dx*dx*sum(sum(Del.*this_joint_I));
    % mean_delta_I_traj(kk) = dx*sum(sum(Del.*this_I_eps_delta_array));


    this_R_eps_delta_array(:,:) = R_traj_eps_delta_array(kk,:,:);
        
    % this_joint_R = this_R_eps_delta_array/R_traj(kk)/dx/dx;
    this_joint_R(:,:) = reshape(R_traj_eps_delta_array(kk,:,:),m,n)/R_traj(kk)/dx/dx;
    joint_R_traj(kk,:,:) = this_joint_R;


end
% delta_S_dist_traj = sum(del.*marginal_delta_S_traj,2)./S_traj;
% mean_delta_S_traj = sum(delta_S_dist_traj,2);

% epsilon_I_dist_traj = sum(eps.*marginal_eps_I_traj,2)./I_traj;
% mean_epsilon_I_traj = sum(epsilon_I_dist_traj,2);

% delta_I_dist_traj = sum(del.*marginal_delta_I_traj,2)./I_traj;
% mean_delta_I_traj = sum(delta_I_dist_traj,2);
% mean_delta_I_traj = sum(Del.*I_traj_eps_delta_array,[2,3])./I_traj;
...
if want_to_plt_distributions
    ...
    % this_joint_S(:,:) = reshape(S_traj_eps_delta_array(this_ind,:,:),m,n)/S_traj(this_ind)/dx/dx;
    % this_joint_I(:,:) = reshape(I_traj_eps_delta_array(this_ind,:,:),m,n)/I_traj(this_ind)/dx/dx;
    % this_joint_R(:,:) = reshape(R_traj_eps_delta_array(this_ind,:,:),m,n)/R_traj(this_ind)/dx/dx;
