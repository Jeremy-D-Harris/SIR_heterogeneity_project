...
marginal_eps_S_traj = reshape(sum(S_traj_eps_delta_array,2),length(t_span),m+1);
marginal_delta_S_traj = reshape(sum(S_traj_eps_delta_array,3),length(t_span),n+1);
marginal_eps_I_traj = reshape(sum(I_traj_eps_delta_array,2),length(t_span),m+1);
marginal_delta_I_traj = reshape(sum(I_traj_eps_delta_array,3),length(t_span),n+1);
...
results.marginal_delta_I_traj = marginal_delta_I_traj;
results.marginal_delta_S_traj = marginal_delta_S_traj;
results.marginal_eps_S_traj = marginal_eps_S_traj;
...
%% option: save & run classic SIR


%% option: save & run Susceptible Variation SIR
