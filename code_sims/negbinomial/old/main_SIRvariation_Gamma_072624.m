...
if readin_init_joint
    ...
    marg_eps_S = marg_eps_S./sum(marg_eps_S); %normalize
    ...
    marg_delta_I = marg_delta_I./sum(marg_delta_I); %normalize
    ...
    init_values_S = init_joint_S*S_init; %joint probability matrix S_{i,j}
    params.init_values_S = init_values_S;

    init_values_I = init_joint_S*I_init;
    params.init_values_I = init_values_I;

    init_values_R = init_joint_S*R_init;
    params.init_values_R = init_values_R;

    init_conds = [reshape(init_values_S, m*n,1); reshape(init_values_I,m*n,1); reshape(init_values_R, m*n,1)];
...
init_marg_eps_I = sum(init_joint_S);
init_marg_delta_I = sum(init_joint_S,2)';
...
marginal_eps_S_traj = reshape(sum(S_traj_eps_delta_array,3),length(t_span),m);
marginal_delta_S_traj = reshape(sum(S_traj_eps_delta_array,2),length(t_span),n);
marginal_eps_I_traj = reshape(sum(I_traj_eps_delta_array,3),length(t_span),m);
marginal_delta_I_traj = reshape(sum(I_traj_eps_delta_array,2),length(t_span),n);

% epsilon_S_dist_traj = sum(eps.*marginal_eps_S_traj,2)./S_traj;
% mean_epsilon_S_traj = sum(epsilon_S_dist_traj,2);
for kk=1:length(t_span)
    this_S_eps_delta_array(:,:) = S_traj_eps_delta_array(kk,:,:);
    mean_epsilon_S_traj(kk) = sum(sum(Eps.*this_S_eps_delta_array))./S_traj(kk);

    this_I_eps_delta_array(:,:) = I_traj_eps_delta_array(kk,:,:);
    mean_delta_I_traj(kk) = sum(sum(Del.*this_I_eps_delta_array))./I_traj(kk);
