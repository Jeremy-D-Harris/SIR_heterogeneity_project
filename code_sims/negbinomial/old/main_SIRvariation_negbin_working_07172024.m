...
if 1
    ...
    this_ind = 1;
    subplot(1,2,1);
    this_joint_S(:,:) = reshape(S_traj_discrete(this_ind,:,:),m,n)/S_traj(this_ind);
    % imagesc(eps_plt,del,this_joint_S);
    imagesc(eps_plt,delta_plt,this_joint_S);
    % pcolor(eps,del,joint_S);
    ...
    subplot(1,2,2);
    this_joint_I = reshape(I_traj_discrete(this_ind,:,:),m,n)/I_traj(this_ind);
    imagesc(eps_plt,delta_plt,this_joint_I);
    % pcolor(eps,del,joint_S);
    %axis xy;
    set(gca,'YDir','normal');
    colorbar;
