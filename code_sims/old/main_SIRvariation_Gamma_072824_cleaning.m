...
for kk=1:length(t_span)
    ...
    variance_eps_S_traj(kk,1) = sum((eps- mean_eps_S_traj(kk)*ones(size(eps))).^2.*marginal_eps_S_traj(kk,:));
    variance_delta_S_traj(kk,1) = sum((delta- mean_delta_S_traj(kk)*ones(size(del))).^2.*marginal_delta_S_traj(kk,:));
    ...
    variance_delta_I_traj(kk,1) = sum(((del- mean_delta_I_traj(kk)*ones(size(del))).^2).*marginal_delta_I_traj(kk,:));
...
if run_variation_susc_SIR
    ...
    for kk=1:length(t_span)
        ...
        variance_eps_S_traj_var_susc(kk,1) = sum((eps- mean_epsilon_S_traj_var_susc(kk)*ones(size(eps))).^2.*marginal_eps_S_traj_var_susc(kk,:));
...
results.mean_epsilon_S_traj = mean_eps_S_traj;
