...
figure_name = 'Figure5_negbinovertime_092022';
...
var_eps = fliplr([0.3 0.5 0.70 0.999]);
...
for count = 1:2
    ...
    if count==1
        % (1) positive
%         infile_independent = 'negbinomial_Poissonlike.mat';
        infile_independent = 'negbinomial_overdispersed_uncorr.mat';
        load(strcat(file_location,infile_independent));
        
        
    elseif count==2
        % (2) independent
%         infile_positivecorrelations = 'negbinomial_overdispersed_uncorr.mat';
        infile_positivecorrelations = 'negbinomial_overdispersed_poscorr.mat';
        load(strcat(file_location,infile_positivecorrelations));
        
        
    else
        % (3) negative
        infile_negativecorrelations = 'negbinomial_overdispersed_poscorr.mat';
...
box('off');
% this_FOS_v
this_p(1) = plot(t_shift_plt(1,:)+15, this_FOS_v,'Color',[0.5 0.5 0.5],'LineWidth',2.5); hold on;
for count = 1:2
...
for kk=1:length(var_eps)
    ...
    if kk==1
        text(0.6,0.93,txt_eps1,'Interpreter','Latex','Units','normalized','FontSize',11);
    elseif kk==2
        text(0.6,0.65,txt_eps2,'Interpreter','Latex','Units','normalized','FontSize',11);
    elseif kk==3
        text(0.6,0.45,txt_eps3,'Interpreter','Latex','Units','normalized','FontSize',11);
    else
        text(0.6,0.25,txt_eps4,'Interpreter','Latex','Units','normalized','FontSize',11);
    end
end


axis([0 this_t_end_plt 0.1 1.1]);
