...
figure_name = 'Figure_correlations_distributionsovertime_matchR0_NegBin';
...
for count_eps = 1:length(epsilon_level)
    ...
    for count=1:2
        ...
        plt_this_susceptibility_distribution(1,:) = collect_marginal_susceptibility(count_eps,3-count,:);

        subplot(4,3,(2+3*(count_eps-1)));
        this_q=plot(eps,plt_this_susceptibility_distribution,'.-','Color',colors_rgb(3-count,:),'LineWidth',3/count,'MarkerSize',18/count); hold on;
        % this_q.Color(4)=0.85;

        if count==1
            plot(this_epsilon_level,0,'.','Color',colors_rgb(3-count,:),'LineWidth',2,'MarkerSize',20);
        elseif count==2
            plot(this_epsilon_level,0,'o','Color',colors_rgb(3-count,:),'LineWidth',2,'MarkerSize',8);
        else
            plot(this_epsilon_level,0,'o','Color',colors_rgb(3-count,:),'LineWidth',2,'MarkerSize',12);
        end
...
for count_eps = 1:length(epsilon_level)
    ...
    for count=1:2
        ...
        this_collect_mean_delta_int = mean_delta_I_int(count_eps,3-count);

        %         collect_marginal_transmissibility(count_eps,count,:)
        plt_this_transmissibility_distribution(1,:) = collect_marginal_transmissibility(count_eps,3-count,:);

        this_q=plot(eps,plt_this_transmissibility_distribution,'.-','Color',colors_rgb(3-count,:),'LineWidth',2.5,'MarkerSize',18); hold on;
        this_q.Color(4)=0.8;

        plot(this_collect_mean_delta_int, 0,'.','Color',colors_rgb(3-count,:),'LineWidth',2,'MarkerSize',30);
